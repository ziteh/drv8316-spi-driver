...
typedef enum
{
  f_20khz, /* Freqency of PWM at 100% duty cycle is 20 kHz. */
  f_40khz  /* Freqency of PWM at 100% duty cycle is 40 kHz. */
} drv8316_pwm100dc_freq_t;

typedef enum
{
  ovp_34v, /* VM overvoltage level is 34 V. */
  ovp_22v  /* VM overvoltage level is 22 V. */
} drv8316_ovp_t;

typedef enum
{
  ocp_deglitch_0_2us,  /* OCP deglitch time is 0.2 µs. */
...
typedef enum
{
  ocp_retry_5ms,  /* OCP retry time is 5 ms. */
  ocp_retry_500ms /* OCP retry time is 500 ms. */
} drv8316_ocp_retry_t;
typedef enum
{
  ocp_16a, /* OCP level is 16 A. */
  ocp_24a  /* OCP level is 24 A. */
} drv8316_ocp_lv_t;

typedef enum
{
  fet,   /* Current recirculation through FETs (Brake Mode). */
  diodes /* Current recirculation through diodes (Coast Mode). */
} drv8316_i_lim_recir_t;

typedef enum
{
  x0_15, /* Current sense amplifier's gain is 0.15 V/A. */
...
typedef enum
{
  cl_600ma, /* Buck regulator current limit is set to 600 mA. */
  cl_150ma  /* Buck regulator current limit is set to 150 mA. */
} drv8316_buck_cl_t;

typedef enum
{
  buck_3v3, /* Buck regulator voltage is 3.3 V. */
...
typedef enum
{
  dly_0_0us, /* Driver delay compensation target is 0 µs. */
